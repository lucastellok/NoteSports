...
def init_sqlite_db():
    ...
    if cursor.fetchone()[0] == 0:
        ...
        cursor.executemany(
            "INSERT INTO quadras (nome, local, tipo) VALUES (?, ?, ?)", quadras_padrao
        )
    ...
    for _ in range(max_attempts):
        ...
        try:
            with get_db_connection() as (conn, db_type):
                ...
                if db_type == "mysql":
                    cursor.execute(
                        "SELECT id FROM reservas WHERE codigo_unico = %s", (codigo,)
                    )
                else:
                    cursor.execute(
                        "SELECT id FROM reservas WHERE codigo_unico = ?", (codigo,)
                    )
...
def get_or_create_user(nome, telefone):
    ...
    try:
        with get_db_connection() as (conn, db_type):
            ...
            if db_type == "mysql":
                cursor.execute(
                    "SELECT id FROM usuarios WHERE telefone = %s", (telefone,)
                )
    ...
    try:
        with get_db_connection() as (conn, db_type):
            if db_type == "mysql":
                ...
                cursor.execute(
                    "SELECT id, nome, local, tipo FROM quadras WHERE ativa = TRUE"
                )
                quadras = cursor.fetchall()
            else:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT id, nome, local, tipo FROM quadras WHERE ativa = 1"
                )
                rows = cursor.fetchall()
                quadras = [
                    {"id": r[0], "nome": r[1], "local": r[2], "tipo": r[3]}
                    for r in rows
                ]
    ...
    try:
        ...
        for hora in range(8, 24):
            ...
            horarios_fallback.append(
                {"hora_inicio": hora_inicio, "hora_fim": hora_fim, "disponivel": True}
            )
